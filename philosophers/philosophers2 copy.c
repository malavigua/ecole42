...
t_philosopher	*create_philosopher(int num, pthread_mutex_t *forks,
					int nb_philos)
{
	t_philosopher	*philo;
	struct timeval	tv;
	...
	philo->left_fork = forks[num - 1];
	if (num == 1)
		philo->right_fork = forks[nb_philos -1];
	else
		philo->right_fork = forks[num - 2];
	return (philo);
}
...
int	main(int argc, char **argv)
{
	...
	while (i < table->nb_of_philosophers)
	{
		philos[i] = create_philosopher(i + 1, table->forks,
